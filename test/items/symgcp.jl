...
@testitem "gradients-fullsym" begin
    ...
    @testset "N=3, r=$r, sz=$sz" for r in [1, 2, 5], sz in [3, 10, 50]

        # Form tensor
        U_star = randn(sz, r)
        X = reshape(khatrirao(U_star, U_star, U_star) * ones(r), (sz, sz, sz))
        ...
        @test isapprox(computed_grad_solution_simplified_λ, zeros(eltype(computed_grad_solution_simplified_λ), size(computed_grad_solution_simplified_λ)), atol=1e-10)
    ...
    @testset "N=4, r=$r, sz=$sz" for r in [1, 5], sz in [3, 20]

        # Form tensor
        U_star = randn(sz, r)
        X = reshape(khatrirao(U_star, U_star, U_star, U_star) * ones(r), (sz,sz,sz,sz))
        ...
        M_star = SymCPD(ones(r), (U_star,), (1, 1, 1, 1))
        @test isapprox([M_star[I] - X[I] for I in CartesianIndices(X)], zeros(eltype(X), size(X)), atol=1e-10)
        ...
        function form_fullsym_M(U_λ_vec::Vector{T}) where {T}
            U = reshape(U_λ_vec[1:sz*r], (sz, r))
            λ = U_λ_vec[sz*r+1:end]
            return reshape(khatrirao(U, U, U, U) * λ, (sz, sz, sz, sz))
        end
        objective(Uλ_vec) = norm(X - form_fullsym_M(Uλ_vec))^2
...
@testitem "gradients-partialsym" begin
    ...
    @testset "3-way, r=$r, sz1=$sz1, sz2=$sz2" for r in [1, 5], sz1 in [3, 50], sz2 in [5, 40]

        # Form tensor 
        U1_star = randn(sz1, r)
        U2_star = randn(sz2, r)
        λ_star = ones(r)
        X = reshape(khatrirao(U2_star, U1_star, U1_star) * λ_star, (sz1, sz1, sz2))
        ...
        M_star = SymCPD(λ_star, (U1_star, U2_star), S)
        @test isapprox([M_star[I] - X[I] for I in CartesianIndices(X)], zeros(eltype(X), size(X)), atol=1e-10)
        ...
        @test isapprox(computed_grad_solution_λ, zeros(eltype(computed_grad_solution_λ), size(computed_grad_solution_λ)), atol=1e-10)
        @test isapprox(computed_grad_solution_simplified_U1, zeros(eltype(computed_grad_solution_simplified_U1), size(computed_grad_solution_simplified_U1)), atol=1e-10)
        @test isapprox(computed_grad_solution_simplified_U2, zeros(eltype(computed_grad_solution_simplified_U2), size(computed_grad_solution_simplified_U2)), atol=1e-10)
        @test isapprox(computed_grad_solution_simplified_λ, zeros(eltype(computed_grad_solution_simplified_λ), size(computed_grad_solution_simplified_λ)), atol=1e-10)
        ...
        @test isapprox(computed_grad_λ, auto_grad_λ, rtol=1e-6)
        @test isapprox(computed_grad_simplified_U1, auto_grad_U1, rtol=1e-6)
        @test isapprox(computed_grad_simplified_U2, auto_grad_U2, rtol=1e-6)
        @test isapprox(computed_grad_simplified_λ, auto_grad_λ, rtol=1e-6)

    end

    @testset "4-way, r=$r, sz1=$sz1, sz2=$sz2" for r in [1, 5], sz1 in [3], sz2 in [5]

        # Form tensor 
        U1_star = randn(sz1, r)
        U2_star = randn(sz2, r)
        λ_star = ones(r)
        X = reshape(khatrirao(U2_star, U2_star, U1_star, U1_star) * λ_star, (sz1, sz1, sz2, sz2))
    ...
    @testset "5-way, r=$r, sz1=$sz1, sz2=$sz2" for r in [1, 5], sz1 in [3], sz2 in [5], sz3 in [10]
        ...
        U3_star = randn(sz3, r)
        λ_star = ones(r)
        X = reshape(khatrirao(U3_star, U3_star, U2_star, U1_star, U1_star) * λ_star, (sz1, sz1, sz2, sz3, sz3))
        ...
        M_star = SymCPD(λ_star, (U1_star, U2_star, U3_star), S)
        @test isapprox([M_star[I] - X[I] for I in CartesianIndices(X)], zeros(eltype(X), size(X)), atol=1e-10)
        ...
        @test isapprox(computed_grad_solution_simplified_U1, zeros(eltype(computed_grad_solution_simplified_U1), size(computed_grad_solution_simplified_U1)), atol=1e-10)
        @test isapprox(computed_grad_solution_simplified_U2, zeros(eltype(computed_grad_solution_simplified_U2), size(computed_grad_solution_simplified_U2)), atol=1e-10)
        @test isapprox(computed_grad_solution_simplified_U3, zeros(eltype(computed_grad_solution_simplified_U3), size(computed_grad_solution_simplified_U3)), atol=1e-10)
        @test isapprox(computed_grad_solution_simplified_λ, zeros(eltype(computed_grad_solution_simplified_λ), size(computed_grad_solution_simplified_λ)), atol=1e-10)

        function form_partialsym_M(U_λ_vec)
            U1 = reshape(U_λ_vec[1:sz1*r], (sz1, r))
            U2 = reshape(U_λ_vec[sz1*r+1:(sz1+sz2)*r], (sz2, r))
            U3 = reshape(U_λ_vec[(sz1+sz2)*r+1:(sz1+sz2+sz3)*r], (sz3, r))
            λ = U_λ_vec[(sz1+sz2+sz3)*r+1:end]
            return reshape(khatrirao(U3, U3, U2, U1, U1) * λ, (sz1, sz1, sz2, sz3, sz3))
        end
        objective(Uλ_vec) = norm(X - form_partialsym_M(Uλ_vec))^2
        ...
        init = default_init_sym(X, r, loss, constraints, algorithm, S)
        M0 = deepcopy(init)
        GU = (similar(M0.U[1]), similar(M0.U[2]), similar(M0.U[3]), similar(M0.λ))
        ...
        auto_grad_U1 = reshape(auto_grad[1:sz1*r], size(M_star.U[1]))
        auto_grad_U2 = reshape(auto_grad[sz1*r+1:(sz1+sz2)*r], size(M_star.U[2]))
        auto_grad_U3 = reshape(auto_grad[(sz1+sz2)*r+1:(sz1+sz2+sz3)*r], size(M_star.U[3]))
        auto_grad_λ = auto_grad[(sz1+sz2+sz3)*r+1:end]
        ...
        @test isapprox(computed_grad_simplified_U1, auto_grad_U1, rtol=1e-6)
        @test isapprox(computed_grad_simplified_U2, auto_grad_U2, rtol=1e-6)
        @test isapprox(computed_grad_simplified_U3, auto_grad_U3, rtol=1e-6)
        @test isapprox(computed_grad_simplified_λ, auto_grad_λ, rtol=1e-6)
